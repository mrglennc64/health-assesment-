        ...
        {result && (
          <div>
            ...
            <div style={{ background: "var(--paper-2)", borderRadius: 10, padding: "16px 18px", marginBottom: 24 }}>
              <div className="mono" style={{ fontSize: 10.5, color: "var(--muted-2)", letterSpacing: "0.06em", marginBottom: 6 }}>QUERY</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
              {result.output.clauses?.map((c, i) => (
                <div key={i} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 10, padding: "14px 18px" }}>
                  <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>
                    {c.framework} <span style={{ color: "var(--accent)" }}>{c.citation}</span>
                  </div>
                  {c.note && (
                    <div style={{ fontSize: 13, color: "var(--ink-2)", lineHeight: 1.55 }}>{c.note}</div>
                  )}
                </div>
              ))}
            </div>

            <h2 style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", color: "var(--muted-2)", margin: "0 0 12px" }}>Notes</h2>
